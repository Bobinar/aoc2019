...
def find_best_path_recursive(current_key,keys_left,keys_to_all):
    if (current_key,keys_left) in memo.keys():
        return memo[(current_key,keys_left)]
    ...
    for candidate_key in candidate_keys:  # keys_to_consider.keys():
        ...
        if can_go_there:
            ...
            rest_of_steps, combination = find_best_path_recursive(candidate_key,new_keys_left,keys_to_all)
    ...
    memo[(current_key,keys_left)] = (min_steps, best_combination)
...
if __name__ == '__main__':
    ...
    image = []
    with open('day_18.txt', 'r') as input_file:
    ...
    keys_to_all['@'] = search_all_reachable_keys(image, starting_point)
    ...
    for i in range(len(keys_all)):
        key_cosa = key_cosa + '1'
    print(find_best_path_recursive(current_key, key_cosa,keys_to_all))
    '''
    pending_paths = [(current_key,set(),0)]
    print(len(keys_all))
    while len(pending_paths) > 0:
        current_key, keys_owned, current_steps = pending_paths.pop()

        if current_steps > min_steps:
            continue
        #print(len(keys_owned))
        if len(keys_owned) == len(keys_all):
            if current_steps < min_steps:
                print(current_steps)
                print(keys_owned)
        ...
        keys_to_try = keys_to_consider.keys()
        keys_to_try = keys_to_try - keys_owned
        keys_to_try = list(keys_to_try)
        keys_to_try.sort(key=lambda x: keys_to_consider[x][0], reverse=True)

        for candidate_key in keys_to_try:#keys_to_consider.keys():
            if candidate_key in keys_owned:
                continue
            distance, keys_needed = keys_to_consider[candidate_key]
            can_go_there = True
            for key_needed in keys_needed:
                if key_needed.lower() not in keys_owned:
                    can_go_there = False
                    break
            if can_go_there:
                new_keys_owned = keys_owned.copy()
                new_keys_owned.add(candidate_key)
                new_steps = current_steps + distance
                pending_paths.append((candidate_key, new_keys_owned,new_steps))
    ...
    while len(pending_paths) > 0:
        ...
        current_position, current_keys, current_steps = pending_paths.pop()
        if iter_count % 100000 == 0:
            print('------ iter = ' + str(iter_count) + ' keys = ' + str(current_keys))

        if current_steps > min_steps:
            continue
        if len(current_keys) < len(keys_all):
            reachable_keys_to_moves = search_all_reachable_keys(image,current_position,current_keys)

            keys_to_try = list(reachable_keys_to_moves.keys())
            keys_to_try.sort(key=lambda x: reachable_keys_to_moves[x],reverse=True)
            for reachable_key in keys_to_try:
                if current_steps + reachable_keys_to_moves[reachable_key] > min_steps:
                    continue
                new_keys = current_keys.copy()
                new_keys.append(reachable_key)
                pending_paths.append((key_locations[reachable_key],new_keys,current_steps + reachable_keys_to_moves[reachable_key]))
        else:
            if current_steps < min_steps:
                print(current_steps)
                print(current_keys)
            ...
            min_steps = min(min_steps,current_steps)
    '''
    # idea: LUT => to get from key a to key b, distance is c, need keys k1,k2,..kn. what about when new path opens?
